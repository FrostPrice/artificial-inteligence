...
public class Mapa {
			...
			if (BytesPorTiles == 1) {
				...
				for (int j = 0; j < Altura; j++) {
					for (int i = 0; i < Largura; i++) {
						...
						dado = ((int) b1 & 0x00ff) | (((int) b2 & 0x00ff) << 8) | (((int) b3 & 0x00ff) << 16)
								| (((int) b4 & 0x00ff) << 24);
	...
	public void DesenhaSe(Graphics2D dbg) {
		...
		for (int j = 0; j < NumeroTilesY + somay; j++) {
			for (int i = 0; i < NumeroTilesX + somax; i++) {
				...
				dbg.drawImage(TileSet, (i << 4) - offx, (j << 4) - offy, (i << 4) + 16 - offx, (j << 4) + 16 - offy,
	...
	public int ReadCInt(DataInputStream data) throws IOException {
		...
		return dado = ((int) b1 & 0x00ff) | (((int) b2 & 0x00ff) << 8) | (((int) b3 & 0x00ff) << 16)
				| (((int) b4 & 0x00ff) << 24);
	}
